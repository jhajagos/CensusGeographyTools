  ...
  geographic_unit varchar(255));\n\n""" % schema
    ...
    for table in acs_dict:
        ...
        for table_info in [estimate_table_info, margin_of_error_table_info]:
            ...
            sql_script += "table_id, table_name, file_type, reference_year, period_covered, base_version_table_name, geographic_unit)"
            sql_script += "\n   values ('%(table_id)s','%(table_name_escaped)s','%(file_type)s', '%(reference_year)s', '%(period_covered)s', '%(base_version_table_name)s', '%(geographic_unit)s');\n\n" % table_info
        ...
        CSV HEADER;\n\n""" % (full_base_table_name, os.path.abspath(full_path_file_name))
            ...
            sql_script += "create index idx_%s_geoid on %s(geoid);\n" % (base_table_name, full_base_table_name)
